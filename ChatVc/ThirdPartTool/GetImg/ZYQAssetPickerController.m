...
//#define kThumbnailLength    (375.0f-10.0f)/4
//#define kThumbnailSize      CGSizeMake(kThumbnailLength, kThumbnailLength)
CGSize kThumbnailSize;
CGFloat kThumbnailLength;
...
- (void)setupButtons
{
    ...
    NSLog(@"self.navigationItem.rightBarButtonItem.customView:%@",self.navigationItem.rightBarButtonItem.customView);
    NSLog(@"self.navigationItem.rightBarButtonItem: %@",self.navigationItem.rightBarButtonItem);

    self.navigationItem.rightBarButtonItem.tintColor=[UIColor whiteColor];
#warning 怎么修改颜色返回按钮的颜色
...
- (void)setupAssets
{
    ...
    titleText.text=[self.assetsGroup valueForProperty:ALAssetsGroupPropertyName];
    NSLog(@"%@  ",[self.assetsGroup valueForProperty:ALAssetsGroupPropertyName]);
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (indexPaths.count == 0)
    {
        ...
        titleText.textColor=[UIColor whiteColor];
        [titleText setFont:[UIFont systemFontOfSize:19.0]];
        [titleText setText:[self.assetsGroup valueForProperty:ALAssetsGroupPropertyName]];
        self.navigationItem.titleView=titleText;
        //  将右上角设置成取消
        self.navigationItem.rightBarButtonItem =
        [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"取消", nil)
    ...
    NSLog(@"%@  %d " ,self.navigationItem.rightBarButtonItem.title,indexPaths.count);
    self.navigationItem.rightBarButtonItem =
...
#warning 为什么push后导航栏颜色会浓一些，为什么返回按钮颜色不变
        ...
        //        rightButton.titleLabel.text=@"asdf";
        //        rightButton.tintColor=[UIColor yellowColor];
        //        //    rightButton.titleLabel.text=@"完成";
...
+(void)initSize:(float)mySize{
    kThumbnailLength=mySize;
    kThumbnailSize=CGSizeMake(kThumbnailLength, kThumbnailLength);
}
- (id)init
{
