...
-(int)getR1:(NSString *)upsql :(NSMutableArray *)BindDate{
    ...
    if(sqlite3_open([[self pathnameofDb] UTF8String],&db)!=SQLITE_OK){
        NSLog(@"初始化成就表打开数据库失败");
        return -1;
    }    else{
...
-(NSString *)pathnameofDb{
    
//    StatusOfKeyboard.Emoji
...
//                        [r addObject:[NSString stringWithFormat:@"%d",sqlite3_column_int(statement, i)]];
                         [r addObject:[NSNumber numberWithInt:sqlite3_column_int(statement, i)]];
                    }
                    else
                    {
...
-(int)addDatabase{
    ...
    NSString *writableDBPath = [self pathnameofDb];
    success = [fileManager fileExistsAtPath:writableDBPath];
    if (success) {databaseIsReady=1; NSLog(@"已经存在"); return 1;}
    else{
        // The writable database does not exist, so copy the default to the appropriate location.
        NSString *defaultDBPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"storage"];
